...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadTransactions(String userId) async {
    ...
    _todayExpenses = allTransactions
        .where((t) => t.type == 'expense' && DateTime.parse(t.date).isAfter(todayStart))
  ...
  Widget _buildDailyLimitSection() {
    ...
    final progress = _dailyLimit > 0 ? _todayExpenses / _dailyLimit : 0.0;
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text('Spent: $_currencySymbol ${currencyFormatter.format(_todayExpenses)}'),
              Text('Limit: $_currencySymbol ${currencyFormatter.format(_dailyLimit)}'),
            ],
          ),
          const SizedBox(height: 4),
          LinearProgressIndicator(
            value: progress.clamp(0.0, 1.0), // Clamp value to prevent errors
            minHeight: 10,
                                ...
                                child: TextButton(
                                  ...
                                  onPressed: () async {
                                    ...
                                    final billTransaction = model.Transaction(
                                      ...
                                      categoryId: await _getOrCreateBillCategory('Bills', _currentUser!.uid),
                                    );
                                    await dbHelper.addTransaction(billTransaction, _currentUser!.uid);
      ...
      itemBuilder: (context, index) {
        ...
        return Dismissible(
          ...
          onDismissed: (direction) {
            if (_currentUser != null) {
              _deleteTransaction(transaction.id!, _currentUser!.uid);
            }
          },
